...
class PolicyValueNet(nn.Module):
    """Small inspectable chess policy/value network."""
...
class PolicyValueTrainer:
    ...
    def train_batch(self, examples: list[TrainingExample]) -> TrainStats:
        if not examples:
        ...
        policy_loss = masked_policy_loss(logits, examples)
        value_targets = torch.tensor([ex.value_target for ex in examples], dtype=torch.float32)
...
def masked_policy_loss(logits: torch.Tensor, examples: list[TrainingExample]) -> torch.Tensor:
    """Cross entropy over legal/search moves only, matching PUCT inference."""

    losses = []
    for row, example in enumerate(examples):
        moves = tuple(example.policy_target)
        indices = torch.tensor([action_index(move) for move in moves], dtype=torch.long)
...
class NeuralPolicyValueEvaluator:
    """Model adapter for PUCT: legal priors plus White-perspective value."""
    ...
    def evaluate(self, board: chess.Board) -> tuple[dict[str, float], float]:
        ...
        state = encode_board_ascii(board_to_ascii(board), board.turn).unsqueeze(0)
        logits, values = self.model(state)
        indices = torch.tensor([action_index(move) for move in legal_moves], dtype=torch.long)
