...
### 2026-05-27 18:17:19 UTC — Removed side-to-move model input plane

- Change: removed the side-to-move/turn plane from the neural board encoder; the model now receives only the 12 visual piece planes from the Unicode board diagram. `python-chess` board state still supplies legal moves and side-to-move for search, target signing, and validation accounting.
- TDD red command: `uv run pytest tests/test_core.py::test_board_encoder_preserves_visual_state_without_side_to_move_plane -q`
- Red result: failed as expected because `PolicyValueNet.encode_board_ascii()` still required a `turn` argument.
- Targeted green command: `uv run pytest tests/test_core.py::test_board_encoder_preserves_visual_state_without_side_to_move_plane tests/test_core.py::test_model_uses_a_deeper_residual_tower tests/test_core.py::test_policy_value_trainer_reduces_loss_on_repeated_target -q`
- Targeted green result: passed (`3 passed in 2.58s`).
- Full verification command: `uv run pytest -q`
- Full verification result: passed (`24 passed, 1 warning in 61.85s`).
